"""Dark glassmorphism over a photographic backdrop.

Built to the rules in uxpilot.ai/blogs/glassmorphism-ui, applied to a near-black
photograph (bubbles in water) rather than a gradient.

Why this is a dark theme. The supplied image is effectively black - mean RGB
(4,4,4), median luminance 0.00, with sparse specular highlights on the bubbles.
Light panels over it would grey the photo out and hide the only part worth
seeing. The article's dark-mode guidance is what applies: darker tints, lighter
text, gentler shadows, and "a subtle colored rim (navy, violet, or teal)
instead of pure white".

The numbers, all measured rather than chosen by eye:

  * Scrim. rgba(7,10,18,.55) over the photo. This pulls the bubble highlights
    from 240 down to ~112 so no panel ever straddles a hard light-to-dark jump,
    while leaving the bubbles clearly visible - they are the image.
  * Panels. #0E1320 at .72 behind an 18px blur. Over the brightest highlight
    that resolves to #292D38; over black, #0B0F1A.
  * Contrast, worst case (a panel sitting on a bubble highlight): ink 12.4:1,
    ink-2 8.7:1, ink-3 5.4:1, green 7.9:1, amber 8.2:1, red 5.1:1, blue 5.8:1 -
    all clearing WCAG AA 4.5:1 for small text, with more margin over black.
  * Separation. A dark panel on a black photo differs from its canvas by only
    ~1.06:1 in fill, and no realistic opacity fixes that. So depth is carried
    by the border and rim light instead, exactly as the article prescribes for
    dark mode - a cool-toned rim, never pure white.
  * Blur. 18px on primary panels, 12px on dense ones. The image is also
    pre-blurred at build time, which the article notes cuts the runtime cost of
    the CSS filter.
  * Selective use. Glass on the sidebar, cards, hero, alerts, toasts, tabs and
    sign-in. Tables and forms sit at .88 so dense data stays sharp.
  * Motion. Glass panels reveal with opacity and lift only - the article warns
    against animating blur on something already running a backdrop-filter.
"""
...
<style>
  :root {
    /* text - measured against a panel sitting on a bubble highlight */
    --ink:   #F2F4F8;   /* 12.4:1 */
    --ink-2: #C7CEDC;   /*  8.7:1 */
    --ink-3: #9AA3B8;   /*  5.4:1 */

    /* glass */
    --glass:        rgba(14, 19, 32, .72);
    --glass-strong: rgba(14, 19, 32, .82);
    --glass-dense:  rgba(12, 16, 27, .88);
    --edge:         rgba(255, 255, 255, .13);
    --edge-soft:    rgba(255, 255, 255, .08);
    /* cool rim, not pure white - the article's dark-mode note */
    --rim:          inset 0 1px 0 rgba(150, 190, 255, .16);

    --blur:       saturate(140%) blur(18px);
    --blur-dense: saturate(125%) blur(12px);
    --blur-light: saturate(130%) blur(10px);
    ...
    --amber: #FBBF24;
    --red:   #FB7185;
    --green-tint: rgba(74, 222, 128, .16);
    --amber-tint: rgba(251, 191, 36, .16);
    --red-tint:   rgba(251, 113, 133, .16);
    --grey-tint:  rgba(160, 180, 220, .11);

    --r: 20px;
    --r-lg: 28px;
    --r-sm: 14px;

    /* one light source, top-left; gentler shadows than light mode */
    --shadow:    0 2px 8px rgba(0, 0, 0, .34), 0 14px 38px rgba(0, 0, 0, .40);
    --shadow-hi: 0 4px 14px rgba(0, 0, 0, .40), 0 22px 56px rgba(0, 0, 0, .50);
    --ease: cubic-bezier(.32, .72, 0, 1);
    --font: "Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif;
  }

  /* ---------- photographic backdrop ----------
     Fixed, so panels slide over a still image, with a scrim so the bubble
     highlights never sit under text at full strength. */
  .stApp {
    background:
      linear-gradient(rgba(7, 10, 18, .55), rgba(7, 10, 18, .55)),
      url("app/static/bg.jpg") center / cover no-repeat fixed,
      #05070C;
  }
  ...
  [data-testid="stSidebarNav"] a[aria-current="page"] {
    background: rgba(255, 255, 255, .13) !important; box-shadow: var(--rim);
  }
  ...
  [data-testid="stSidebar"] [data-testid="stMetric"] { background: rgba(255, 255, 255, .06); }
  ...
  .flux-avatar {
    ...
    color: #0B1020; font-weight: 700; font-size: .92rem;
    letter-spacing: -.02em; border: 2px solid rgba(255, 255, 255, .22);
  ...
  .kpi-badge.down { background: var(--red-tint); color: var(--red); border-color: rgba(251, 113, 133, .3); }
  .kpi-badge.flat { background: var(--grey-tint); color: var(--ink-2); border-color: var(--edge); }
  .kpi-sub { font-size: .775rem; color: var(--ink-3); margin-top: .4rem; line-height: 1.4; }
  .kpi-card.is-red   { background: linear-gradient(180deg, rgba(251,113,133,.15), var(--glass)); }
  .kpi-card.is-amber { background: linear-gradient(180deg, rgba(251,191,36,.14), var(--glass)); }
  .kpi-card.is-green { background: linear-gradient(180deg, rgba(74,222,128,.13), var(--glass)); }
  ...
  .chip-red   { background: var(--red-tint);   color: var(--red);   border-color: rgba(251,113,133,.3); }
  .chip-amber { background: var(--amber-tint); color: var(--amber); border-color: rgba(251,191,36,.3); }
  .chip-green { background: var(--green-tint); color: var(--green); border-color: rgba(74,222,128,.3); }
  ...
  .callout.red   { background: linear-gradient(180deg, rgba(251,113,133,.15), var(--glass)); }
  .callout.amber { background: linear-gradient(180deg, rgba(251,191,36,.14), var(--glass)); }
  .callout.green { background: linear-gradient(180deg, rgba(74,222,128,.13), var(--glass)); }
  ...
  .src-note { color: var(--ink-3); font-size: .775rem; margin-top: .5rem; opacity: .8; }
  ...
  .g-alert {
    ...
    background: linear-gradient(180deg, rgba(251,191,36,.14), var(--glass));
    animation: liftIn .46s var(--ease) both;
  }
  .g-alert.red {
    border-left-color: var(--red);
    background: linear-gradient(180deg, rgba(251,113,133,.15), var(--glass));
  }
  ...
  /* A filtered, transformed or backdrop-filtered ancestor becomes the
     containing block for position:fixed, which would anchor the toast stack to
  ...
  .toast {
    ...
    background: linear-gradient(180deg, rgba(251,113,133,.16), var(--glass-strong));
    animation: toastIn .46s var(--ease) both;
  }
  .toast.amber {
    border-left-color: var(--amber);
    background: linear-gradient(180deg, rgba(251,191,36,.15), var(--glass-strong));
  }
  ...
  [data-baseweb="tab"]:hover { color: var(--ink); background: rgba(255, 255, 255, .07) !important; }
  [data-baseweb="tab"][aria-selected="true"] {
    color: #08101F !important; font-weight: 600;
  ...
  button[data-testid="stBaseButton-primary"] {
    background: linear-gradient(135deg, var(--blue), var(--violet)) !important;
    border: none !important; color: #08101F !important; font-weight: 600;
    box-shadow: 0 4px 16px rgba(125, 168, 255, .26);
  }
  ...
  [role="option"]:hover { background: rgba(255, 255, 255, .09) !important; }
  ...
  [data-testid="stSegmentedControl"] button[aria-pressed="true"] {
    background: linear-gradient(135deg, var(--blue), var(--violet)) !important;
    color: #08101F !important; border-color: transparent !important;
  }
  ...
  [data-testid="stPopoverButton"]:hover { background: rgba(255, 255, 255, .09) !important; }
  ...
  [data-testid="stPopoverBody"] .stButton > button:hover { background: rgba(255, 255, 255, .1) !important; }
  ...
  .signin-mark {
    ...
    background: linear-gradient(140deg, var(--blue), var(--violet) 55%, var(--magenta));
    display: flex; align-items: center; justify-content: center;
    color: #08101F; font-size: 1.45rem; font-weight: 700; letter-spacing: -.035em;
    box-shadow: 0 8px 26px rgba(125, 168, 255, .3);
  }
  ...
  /* ---------- phones ---------- */
  @media (max-width: 640px) {
    /* background-attachment: fixed stutters badly on mobile */
    .stApp { background-attachment: scroll, scroll, scroll; }
    [data-testid="stMain"] .block-container {
    ...
    :root { --blur: saturate(130%) blur(12px); --blur-dense: saturate(120%) blur(9px); }
  }
